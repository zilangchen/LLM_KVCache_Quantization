...
**主线**：behavior-aligned framework 统一原则，INT8 为 foundation，INT4-RoleAlign 为新主方法实例。
**扩展**：MixedKV 为诊断驱动实用扩展，INT4-RoleAlign 为低比特路径中的核心 extension。
...
| **int4_ours_asym** | 新主方法 primary instance (INT4-RoleAlign) | ch3 §3.X + ch4 §4.5 |
...
### 2026-03-24 | 主线升级：RoleAwareAsym + BA-guided percentile 成为新主方法实例

**决策**：论文主线从"INT8 BA 验证 + INT4 失败报告"升级为
"BA framework 统一原则 → INT8 foundation → INT4 诊断 → RoleAwareAsym 新方法"。

**变更**：
- 新主方法实例：INT4-RoleAlign = role-aware asymmetric quantizer + BA-guided percentile calibration
