...
commitment deadline 以官方公布为准，预计 2026 年 6-8 月；里程碑按 ARR cycle 驱动）
...
| `meta-llama/Llama-3.1-8B-Instruct` | 跨架构验证，证明泛化性（128K 上下文） | 待首次下载后 pin |
...
**RULER 主表子任务**（使用 [NVIDIA/RULER](https://github.com/NVIDIA/RULER) 官方分类术语）：
- **Retrieval**：S-NIAH（单 needle）、MK-NIAH（多 key 干扰）
- **Multi-hop Tracing**：VT（Variable Tracking，链式变量绑定）
