...
| KIVI-style baseline | 受 KIVI 启发的异轴量化对照（per-channel K + per-token V，自实现简化版；与 KIVI 原文差异见下） | ✅ 已实现 |
...
- **INT4 存储说明**：`KIVIStyleKVCache` 的 INT4 路径已采用 nibble bit-packing（0.5 byte/value），
  与 `INT4KVCache` 的存储粒度一致；论文内存表仍需注明 KIVI 与 INT4-ours 的 kernel 路径差异
