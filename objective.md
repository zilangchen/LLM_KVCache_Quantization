...
**主线**：behavior-aligned framework，**主验证 regime** = INT8。
**扩展**：INT4 / KIVI / MixedKV 作为 stronger-compression extension，
验证框架诊断能力在不同 bit-width 下的泛化边界。

| 方法 | 角色 | 论文位置 |
|------|------|---------|
| **int8_ours** | 主框架 primary validated instance | ch4.2 INT8: 校准+融合解码 |
| fp16 / int8_baseline / kivi_style(INT8) | baselines + SOTA 对照 | ch4.2 对照 |
| int4_baseline / int4_ours | 失败对照 | ch4.4 对称 INT4 压力测试 |
| MixedKV (K-INT8/V-INT4) | 诊断驱动的实用扩展 | ch4.5 K/V 不对称诊断 + 实用方案 |
| Phase 0 诊断 / K/V 消融 | 机制分析 | ch4.5 诊断洞察 |
