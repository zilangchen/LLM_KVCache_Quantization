...
先评估运行模式（Execute/Wait/Monitor 三模式切换），再检查退出条件。
详细逻辑参照 `supervisor.md` §三模式状态机 和 §智能熔断与退出条件。

| 条件 | 动作 |
|------|------|
| **所有目标达成** | 输出最终总结，正常退出 |
| **有远程任务运行中** | 切换到 Wait/Monitor 模式，继续本地填充工作或定期监控 |
...
- **无固定轮次上限**，以 `objective.md` Success Criteria 为退出标准
- **单轮超时**：如果单轮执行超过 15 分钟没有产出，自动暂停并汇报
- 详细熔断逻辑参照 `supervisor.md` §智能熔断与退出条件

### 无进展检测（最重要的防护）

每轮结束时自评本轮是否产生了 **实质性进展**（扩展定义：新 commit / 解决 issue / 远程 runs 增长 / 审查报告 / 填充工作完成 均算进展）。

- **Execute 模式连续 3 轮无实质进展** → 熔断退出，输出诊断报告，标记 `<!-- auto-iterate-stalled -->`
- **Wait/Monitor 模式不计入熔断**（等待远程实验是正常行为）
- **连续失败熔断**：同一个目标连续 3 轮都在 Phase 4 失败（`$debug-iterate` 无法修复）→ 停止
