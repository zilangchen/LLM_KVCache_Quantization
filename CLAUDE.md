...
- **主管 Agent**（supervisor.md）：最高权限，拆解目标为任务，驱动迭代循环，仅在目标达成/迭代上限/硬阻塞/连续无进展时退出
- **开发 Agent**（developer.md）：高级权限，执行编码/测试/修复，自主调用 debug/commit/hygiene 流程
- **审查 Agent 集群**（7 个专项 Agent，各司其职）：
  ...
  - 全部只读 + 仅写 `review_tracker.md`，由 Supervisor 并行调度
