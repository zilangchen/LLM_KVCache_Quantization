...
当以 **主管 Agent** 角色运行（Agent Teams 模式 / `/auto-iterate`）时，**ExecPlan 门禁完全豁免**。主管 Agent 及其调用的 Codex Developer 可自主决策并执行，无需输出 ExecPlan、无需等待用户 `APPROVE PLAN`、无需使用 `EnterPlanMode`。仅在触发 `auto-iterate` SKILL.md 中定义的强制 Escalation 场景（修改 objective 目标/边界、破坏性操作、研究方向转变）时才暂停询问用户。
...
- **Developer 隔离**：Developer 角色由 Codex 通过 MCP 工具调用，直接在 main 工作目录中操作（非 sandbox 隔离），不创建分支。Codex 修改后由 Supervisor 审核 `git diff` + `pytest` 验证，通过后在 main 上提交
...
- **开发 Agent / Codex Developer**（developer.md）：由 Codex (GPT-5.4) 执行，Supervisor 通过 MCP 工具 `mcp__codex__codex` 调用。两阶段流程：read-only 分析 → 直接在 main 修复+测试。可读取 tracker（只读）、可访问远程服务器。Codex 不提交代码，由 Supervisor 审核后落地
...
**启动规则**：所有 Claude Code Agent 角色（Supervisor、Review-Coord、D1-D7）启动时必须先读取 `review_tracker.md` 和 `iteration.md`，这是保持上下文同步的唯一机制。Codex Developer 不"启动"——它通过 MCP 工具按需调用，由 Supervisor prompt 提供上下文。
