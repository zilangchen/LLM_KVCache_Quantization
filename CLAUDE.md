...
- **开发 Agent / Codex Developer**（developer.md）：由 Codex (GPT-5.4) 执行，Supervisor 通过 Codex Plugin 命令调用（`/codex:review` 分析、`/codex:rescue` 执行，Review Gate 自动审查）。两阶段流程：read-only 分析 → 直接在 main 修复+测试。可读取 tracker（只读）、可访问远程服务器。Codex 不提交代码，由 Supervisor 审核后落地
