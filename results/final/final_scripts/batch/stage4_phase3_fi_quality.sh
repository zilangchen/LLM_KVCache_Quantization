#!/bin/bash
# Stage 4: Phase 3 FlashInfer adapter 1.5B 质量评测
#
# 前置条件：v_percentile fix 已生效，FlashInfer adapter 在 production calib 下也快了。
# Stage 1 实测：fi 1.5B = 43.73 ± 0.22 ms（比之前 59.91 快 16 ms）。
#
# 完整 39 个测试（PPL 10 + Needle 12 + RULER 12 + LongBench 5），结构同 Stage 3。
...
echo ""; echo "═══ Stage 4: Phase 3 FlashInfer 1.5B quality ═══"
