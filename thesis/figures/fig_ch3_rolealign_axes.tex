\begin{figure}[!htbp]
  ...
  \begin{tikzpicture}[
    ...
    % K side
    \node[font=\bfseries] at (-4.2,3.2) {Key side: ranking};
    ...
    \node[note] (kagg) at (-4.2,-1.6) {沿 token 方向聚合\\每个 channel 独立确定 $(s_K,z_K)$};
    \node[note] (kpk) at (-4.2,-3.0) {$p_K$ percentile\\attention-distribution KL};
    \draw[arr] (kmat.south) -- (kagg.north);
    \draw[arr] (kagg.south) -- (kpk.north);

    % V side
    \node[font=\bfseries] at (4.2,3.2) {Value side: content};
    ...
    \node[note] (vagg) at (4.2,-1.6) {沿 channel 方向聚合\\每个 token 独立确定 $(s_V,z_V)$};
    \node[note] (vpv) at (4.2,-3.0) {$p_V$ percentile\\输出扰动代理};
    \draw[arr] (vmat.south) -- (vagg.north);
    \draw[arr] (vagg.south) -- (vpv.north);
  \end{tikzpicture}
  \caption{\texttt{INT4-RoleAlign} 的角色感知非对称量化轴。Key 侧沿 token 方向聚合、对每个 channel 单独确定非对称量化参数，以优先保持 attention ranking；Value 侧沿 channel 方向聚合、对每个 token 单独确定非对称量化参数，以适应内容侧的动态范围变化。}
