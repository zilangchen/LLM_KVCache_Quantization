\begin{figure}[!htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[font=\bfseries\small] at (-0.18,-1.80) {输出误差分解};
    \node[font=\normalsize] at (-0.18,-2.35) {分布侧传播项 \(+\) 聚合侧传播项};
  ...
  \end{tikzpicture}
  \caption{注意力误差的两条耦合传播路径及其精确代数分解。左侧为 FP16 attention flow，中部为量化扰动，右侧为由 Key 与 Value 误差分别触发的两条传播路径，底部标出输出误差对应的两类传播项。}
