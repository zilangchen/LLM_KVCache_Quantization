...
上述分解直接说明，下游任务感知到的不是 $K$ 与 $V$ 的数值距离本身，而是它们共同诱导的注意力行为偏移。Key 误差先作用于 logits 的相对排序，再经 softmax 放大或重排，进而影响注意力分布；Value 误差改变被聚合的信息内容，其影响强度同时受量化后分布 $\hat a$ 的调制。只从张量重建误差出发分析量化质量，容易忽略 softmax 非线性与加权聚合共同造成的功能失真；把注意力行为作为联合分析对象，后续校准与分配才能建立在更贴近下游功能损伤的基础之上。

本节由单头解码注意力出发，将 KV Cache 量化形式化为注意力行为联合保持问题，并通过输出误差的精确代数分解说明：Key 与 Value 的量化误差分别沿分布侧与聚合侧传播，但两条路径在数学上彼此耦合，不能简单还原为两个互不相关的数值误差项。基于这一理论起点，第 \ref{sec:ch3-motivation-kv} 节将进一步诊断在低比特场景下哪一侧更先触发行为失稳，第 \ref{sec:ch3-framework}--\ref{sec:ch3-allocator} 节则把这一联合目标逐步落成统一框架、路径实例与预算分配机制。
...
\end{equation}
其中 $s^{(l)}$ 表示第 $l$ 层对量化扰动的行为敏感度读数。在本章中，$\mathcal{S}$ 是概念层对象：它刻画“某一层的量化更容易引起注意力行为扰动”的相对强弱，而不预设必须由某一个固定的数值表达式在所有决策层面直接给出。具体而言，在校准层，行为偏移将通过注意力分布侧的工程代理来具体化；在分配层，出于离线复用与成本控制考虑，本文从同一离线校准链路产生的校准产物中读取逐层可聚合的统计读数，形成与校准层同源的逐层敏感度画像。本文后续两层决策共享的是同一离线校准来源与同一行为组织原则，而不是要求二者在数值上使用完全相同的表达式。
...
\end{equation}
其中 $b^\star$ 表示逐层或逐角色的预算决策向量。与校准层不同，分配层回答的是“哪里优先保护”：在总预算受限时，哪些层更值得保留较高精度，哪些层可以接受更激进的压缩。最基础的情形是逐层预算分配；在此之上，还可以扩展到 K/V 角色感知的预算接口，以及进一步发展出基于敏感度画像形状的自动预算建议机制。allocation / \texttt{AutoK} 在本文中被定位为行为引导框架的扩展层，而不是与校准层并列竞争主轴的另一套独立方法。它的理论合法性来自与校准层共享同源的行为敏感度画像，而不是来自一个与校准层割裂的全新目标函数。
...
后续章节不再把这些路径写成彼此独立的技巧，而是沿同一行为引导原则展开两层决策：校准层负责确定如何量化，分配层负责确定哪里应被优先保护，系统层负责把前两者固化为可执行的推理接口。基于这一统一框架，第 \ref{sec:ch3-calibration} 节将把行为偏移具体化为校准层的工程代理与稳健选择准则；第 \ref{sec:ch3-paths} 节给出 \texttt{INT8} 基准路径、对称 \texttt{INT4} 的局限以及 \texttt{INT4-RoleAlign} 的路径实例；第 \ref{sec:ch3-allocator} 节把同源敏感度画像延伸为逐层预算分配与 \texttt{AutoK} 建议机制；第 \ref{sec:ch3-deployment} 节说明这一整套框架如何落成离线校准产物、在线执行管线与系统层开销分析。

\section{行为引导校准目标与参数搜索策略}
\label{sec:ch3-calibration}

第 \ref{sec:ch3-problem} 节将 KV Cache 量化形式化为注意力行为联合保持问题，并指出真正需要被约束的是量化前后注意力分布与聚合输出的联合偏移；第 \ref{sec:ch3-framework} 节则说明，这一联合目标应在统一的行为引导框架下被组织为校准层与分配层两类决策。联合行为目标本身还不是一个简单、可执行、且适合离线搜索的优化对象。本节把前述概念层目标转写为可操作的工程代理与稳健选择准则：先说明为什么采用注意力分布侧的 KL 散度作为校准层的行为代理，再给出统一离线校准流程下的参数搜索与选择纪律。
