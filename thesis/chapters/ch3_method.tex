...
\end{equation}
其中 $\Delta_{\mathrm{beh}}$ 表示量化前后注意力行为的联合偏移度量。需要强调的是，上式仅用于刻画本文希望控制的对象，即注意力行为本身；在后续方法展开中，它会被降解为路径相关、可执行的行为代理：对称路径主要由注意力分布 KL 刻画，而角色感知低比特路径则分别使用 K-path 与 V-path 的分角色代理。
...
\end{equation}
其中 $s^{(l)}$ 表示第 $l$ 层对量化扰动的行为敏感度读数。在本章中，$\mathcal{S}$ 是概念层对象：它刻画“某一层的量化更容易引起注意力行为扰动”的相对强弱，而不预设必须由某一个固定的数值表达式在所有决策层面直接给出。具体而言，在校准层，行为偏移将通过注意力分布侧的可操作代理来具体化；在分配层，出于离线复用与成本控制考虑，本文从同一离线校准链路产生的校准产物中读取逐层可聚合的统计读数，形成与校准层同源的逐层敏感度画像。本文后续两层决策共享的是同一离线校准来源与同一行为组织原则，而不是要求二者在数值上使用完全相同的表达式。
...
后续章节不再把这些路径写成彼此独立的技巧，而是沿同一行为引导原则展开两层决策：校准层负责确定如何量化，分配层负责确定哪里应被优先保护，系统层负责把前两者固化为可执行的推理接口。基于这一统一框架，第 \ref{sec:ch3-calibration} 节将把行为偏移具体化为校准层的可操作代理与稳健选择准则；第 \ref{sec:ch3-paths} 节给出 \texttt{INT8} 基准路径、对称 \texttt{INT4} 的局限以及 \texttt{INT4-RoleAlign} 的路径实例；第 \ref{sec:ch3-allocator} 节把同源敏感度画像延伸为逐层预算分配与 \texttt{AutoK} 建议机制；第 \ref{sec:ch3-deployment} 节说明这一整套框架如何落成离线校准产物、在线执行管线与系统层开销分析。

\section{行为引导校准目标与参数搜索策略}
\label{sec:ch3-calibration}

第 \ref{sec:ch3-problem} 节将 KV Cache 量化形式化为注意力行为联合保持问题，并指出真正需要被约束的是量化前后注意力分布与聚合输出的联合偏移；第 \ref{sec:ch3-framework} 节则说明，这一联合目标应在统一的行为引导框架下被组织为校准层与分配层两类决策。联合行为目标本身还不是一个简单、可执行、且适合离线搜索的优化对象。本节把前述概念层目标转写为可操作代理与稳健选择准则：先说明为什么采用注意力分布侧的 KL 散度作为校准层的行为代理，再给出统一离线校准流程下的参数搜索与选择纪律。
...
\right).
\end{equation}
这里需要明确一点：本文优化的是\emph{分布侧 KL 代理}，而不是直接声称输出误差 $(\hat o-o)$ 与 KL 在数学上等价。KL 的角色是将第 \ref{sec:ch3-problem} 节中的联合行为目标具体化为校准层可执行的行为代理，而不是替代联合行为目标本身。
