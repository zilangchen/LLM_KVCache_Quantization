...
压缩版诊断首先揭示了一个稳定方向：在本文考察的代表性模型中，导致对称低比特路径失稳的主导因素主要来自 Key 精度下降，而非 Value 的同步退化。更具体地说，当 Key 保持在较高精度时，低比特 Value 往往只带来有限的性能损伤；相反，当 Key 被压缩到更低精度后，即便 Value 仍保持较高精度，检索与语言建模行为也可能发生明显崩塌。换言之，低比特场景下最先被破坏的并不是被聚合的内容本身，而是由 Key 决定的 attention ranking 结构。该观察与第 \ref{sec:ch3-problem} 节的误差传播分析一致：Key 扰动一旦改变 logits 的相对排序，softmax 的非线性会进一步放大这种偏移，使注意力分布在功能意义上偏离参考路径；而单独的 Value 扰动虽然同样会进入聚合侧误差项，但其影响通常滞后于分布结构的失稳。

% 图 3-2 建议置于此处
% 图题：图 3-2 K/V 量化敏感性不对称的压缩版诊断与设计启示
% 数据纪律：本图必须是第 4.3.2 节完整证据的压缩视图；主纵轴为 Needle，RULER 仅作图注/脚注支持
\input{figures/fig_ch3_kv_diag_needle}

进一步地，这种不对称性虽然在不同模型家族中呈现为共同方向，但其强度并不完全一致，而是受到模型规模、家族结构及 GQA 配置的共同影响。本文的代表性诊断表明，一些模型在 Key 降至极低精度后更容易出现近乎完全的检索失稳，而另一些具有更高 $H_{kv}$ 或不同头共享结构的模型则表现出更强的鲁棒性。这意味着“Key 主导退化”应被理解为一个跨模型稳定存在的方向性结论，而非不加区分地适用于所有架构的同强度规则。也正因如此，本文后续并不把该观察简化为单一经验法则，而是将其视为低比特路径设计的结构性约束：后文的方法需要既响应这种总体不对称性，又允许不同 family、scale 与 GQA 结构在恢复强度上呈现差异。

基于上述动机诊断，本文将低比特路径的设计目标从“均匀压缩 K 与 V”转化为“优先保护 Key 的表示能力，并据此引入角色差异化的格式升级”。如果导致对称 INT4 失效的主导因素来自 Key 精度下降，那么继续沿用对 K 与 V 一视同仁的对称压缩便缺乏结构依据；更合理的方向，是使量化格式本身对二者的不同功能角色作出响应。基于这一判断，第 \ref{sec:ch3-framework} 节将进一步给出 behavior-guided 统一框架，第 \ref{sec:ch3-paths} 节则把这一设计动机具体实例化为从对称 INT4 向角色感知低比特路径的格式升级，并最终导向 \texttt{INT4-RoleAlign} 的正式定义。
