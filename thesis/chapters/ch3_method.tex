...
\end{equation}
其中 $s^{(l)}$ 表示第 $l$ 层对量化扰动的行为敏感度读数。这里需要强调的是，$\mathcal{S}$ 在本章中首先是一个概念层对象：它刻画“某一层的量化更容易引起注意力行为扰动”的相对强弱，而不预设必须由某一个固定的数值表达式在所有决策层面直接给出。具体而言，在校准层，行为偏移将通过注意力分布侧的工程代理来具体化；在分配层，出于离线复用与成本控制考虑，本文从同一离线校准链路产生的校准产物中读取逐层可聚合的统计读数，形成与校准层同源的逐层敏感度画像。本文后续两层决策共享的是同一离线校准来源与同一行为组织原则，而不是要求二者在数值上使用完全相同的表达式。
...
需要强调的是，本文在本节所统一的并不是“所有路径共享同一个数值评分函数”，而是同一套离线校准流程、离线校准产物接口与稳健选择纪律。对称路径与角色感知低比特路径在候选参数家族上不同，在行为代理的具体定义上也可以不同；但它们都服从同一校准逻辑：以行为保持为原则、在可行域约束下优先控制尾部坏案例，并将最终选择结果固化为可审计的离线校准产物。基于这一边界，第 \ref{sec:ch3-paths} 节将继续把本节给出的流程实例化到 \texttt{INT8} 基准路径与低比特角色感知路径上。
...
\]
其中第一项是分布侧传播路径，第二项是聚合侧传播路径。KL 之所以适合作为校准层代理，不是因为它完整等价于联合行为误差，而是因为它首先直接控制了分布偏移 $(\hat a-a)$ 的大小，从而约束第一条传播路径；与此同时，第二条传播路径中的权重项为 $\hat a_i$，而非与分布无关的常数，因此当量化后分布被更好地保持时，聚合侧误差中的权重漂移也会随之受控。KL 代理的功能不是“单独优化输出误差”，而是以保持注意力排序与概率质量为抓手，对两条误差传播路径同时施加结构性约束。
...
从输出形式上看，离线校准流程统一产生两类结果：一类是\emph{路径特定校准产物}，包括静态缩放参数、zero-point、分位数参数、分组元数据与保护参数等，用于在线推理时的直接读取与执行；不同路径对应的字段集合可以不同，但都服从同一只读产物接口。另一类是\emph{共享行为统计读数}，它们来自同一离线校准链路，并将在第 \ref{sec:ch3-allocator} 节中被进一步组织为逐层行为敏感度画像。校准层不仅回答“如何量化”，也为后续的预算分配提供同源读数。逐头温度校正等附加组件在本文主线中统一降格为诊断性探索项，仅在附录中保留说明；主线方法默认不依赖 \texttt{inv\_tau}。
...
在这一统一框架下，本章进一步将联合行为目标操作化为可执行的离线校准流程。第 \ref{sec:ch3-calibration} 节以注意力分布 KL 散度作为校准层的工程代理，并给出统一的参数搜索与稳健选择纪律；第 \ref{sec:ch3-paths} 节则沿着由保守到激进的顺序，分别给出 INT8 基准路径、对称 \texttt{INT4} 的局限与格式升级动因，以及 \texttt{INT4-RoleAlign} 的角色感知非对称量化实例，同时在方法层面明确了其与 \texttt{KIVI-style} 的设计边界。进一步地，第 \ref{sec:ch3-allocator} 节将同源的行为敏感度画像延伸为逐层预算分配与 \texttt{AutoK} 预算自动建议机制，使行为引导原则从参数选择自然扩展到预算决策。
