...
\end{equation}
其中 $\Delta_{\mathrm{beh}}$ 表示量化前后注意力行为的联合偏移度量。需要强调的是，上式仅用于刻画本文希望控制的对象，即注意力行为本身；在后续方法展开中，它会被降解为路径相关、可执行的工程代理：对称路径主要由 attention-distribution KL 刻画，而 role-aware low-bit path 则分别使用 K-path 与 V-path 的分角色代理。
...
为突出低比特失稳的角色依赖性，本文从第 \ref{sec:exp-kv-sensitivity} 节同源的完整证据中抽取一个 bit-layout 诊断视图。具体地，\texttt{FP16} 作为未量化参考，\texttt{K8V8} 作为高精度量化参考，\texttt{K8V4} 与 \texttt{K4V8} 构成同平均 bit 预算下的角色对照，而 \texttt{K4V4} 则提供对称低比特坍塌锚点。其中，\texttt{K8V4} 与第四章完整诊断中的恢复性配置 \texttt{MixedKV} 对应，用于与后文完整四配置证据对齐。上述五配置视图并不承担完整实验比较职责，其作用只是用最小但足够的 bit-layout 集合回答：在受限位宽预算下，4-bit 究竟应优先落在 Key 还是 Value；完整的跨模型读数、逐任务结果与统计检验仍在第 \ref{sec:exp-kv-sensitivity} 节中统一给出。
...
\end{equation}
其中 $s^{(l)}$ 表示第 $l$ 层对量化扰动的行为敏感度读数。这里需要强调的是，$\mathcal{S}$ 在本章中首先是一个概念层对象：它刻画“某一层的量化更容易引起 attention behavior 扰动”的相对强弱，而不预设必须由某一个固定的数值表达式在所有决策层面直接给出。具体而言，在校准层，behavior 偏移将通过注意力分布侧的工程代理来具体化；在分配层，出于离线复用与成本控制考虑，本文从同一离线校准链路产生的校准产物中读取逐层可聚合的统计读数，形成与校准层同源的逐层敏感度画像。换言之，本文后续两层决策共享的是同一离线校准来源与同一行为组织原则，而不是要求二者在数值上使用完全相同的表达式。
...
需要强调的是，本文在本节所统一的并不是“所有路径共享同一个数值评分函数”，而是同一套离线校准工作流、离线校准产物接口与稳健选择纪律。对称路径与 role-aware low-bit path 在候选参数家族上不同，在行为代理的具体定义上也可以不同；但它们都服从同一校准逻辑：以 behavior 保持为原则、在可行域约束下优先控制尾部坏案例，并将最终选择结果固化为可审计的离线校准产物。基于这一边界，第 \ref{sec:ch3-paths} 节将继续把本节给出的工作流实例化到 \texttt{INT8} 规范路径与低比特 role-aware 路径上。
...
从输出形式上看，离线校准工作流统一产生两类结果：一类是\emph{路径特定校准产物}，包括静态 scale、zero-point、percentile 参数、group metadata 与保护参数等，用于在线推理时的直接读取与执行；不同路径对应的字段集合可以不同，但都服从同一只读 artifact 接口。另一类是\emph{共享行为统计读数}，它们来自同一离线校准链路，并将在第 \ref{sec:ch3-allocator} 节中被进一步组织为逐层行为敏感度画像。换言之，校准层不仅回答“如何量化”，也为后续的预算分配提供同源 readout。逐头温度校正等附加组件在本文主线中统一降格为诊断性探索项，仅在附录中保留说明；主线方法默认不依赖 \texttt{inv\_tau}。
...
\begin{table}[!htbp]
  ...
  \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{1.45cm}>{\raggedright\arraybackslash}p{2.0cm}>{\raggedright\arraybackslash}p{2.35cm}X}
    \toprule
    维度 & \texttt{KIVI-style} & \texttt{INT4-RoleAlign} & 含义 \\
    \midrule
    量化格式 & per-channel K\newline + per-token V & 相同 & 比较建立在同一 role-aware format family 下 \\
    校准来源 & runtime statistics & offline role-aware calibration & 比较的是 calibration philosophy \\
    参数确定 & 即时统计量 & K-path: $p_K$\newline V-path: $p_V$ & 两侧均被纳入可审计的离线接口 \\
    行为代理 & 无显式\newline behavior proxy & K: attention-distribution KL\newline V: 输出扰动代理 & 显式区分 ranking 与 content 两侧 \\
    推理接口 & 无离线产物 & 有固定校准产物 & 便于审计、冻结\newline 和部署一致性 \\
    向 \ref{sec:ch3-allocator} 扩展 & 无共享 profile 接口 & 可共享 behavior\newline sensitivity profile & 可自然延伸到\newline allocator/\texttt{AutoK} \\
    \bottomrule
  \end{tabularx}
\end{table}
...
行为引导分配层与校准层共享同一离线 provenance，但其操作对象并不是在线重新计算的 attention-KL，而是由离线校准链路产出的校准产物所诱导的层级可读数。记第 $l$ 层从 artifact 中读取的 K/V 角色相关 scale 张量为
\begin{equation}
...
最后需要再次诚实说明的是：本文的主系统落地路径是 \texttt{INT8} 融合 kernel。对 \texttt{INT4-RoleAlign} 而言，\texttt{torch\_ref} 在本文中承担 correctness / reference path，用于质量回归与接口核对；与之并行，\texttt{triton\_fused} 以及面向非对称 \texttt{INT4} 的 Triton / FlashInfer 后端构成可用的快速路径与工程实现候选。第三章在这里只说明这些后端的系统角色，而不把其中任何一条直接写成第四章速度结论的默认来源。
