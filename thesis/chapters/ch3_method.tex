...
第 \ref{sec:ch3-calibration} 节已经定义离线校准流程与稳健选择纪律，但这一流程本身不规定唯一位宽或唯一量化格式。本节只定义路径实例，不提前报告第四章的完整比较结果。三条路径按约束强度展开：\texttt{INT8} 基准路径用于检查校准闭环能否在较保守位宽下稳定落地；对称 \texttt{INT4} 试探路径用于暴露直接降位宽后的结构困难；\texttt{INT4-RoleAlign} 则把低比特路径改写为 K/V 角色分离的非对称格式，并在方法层面区分它与 \texttt{KIVI-style} 的参数来源。
...
在自回归解码阶段，单步计算通常只涉及一个新的 query token，但需要访问全部历史 KV Cache，因此瓶颈主要来自访存。朴素路径会先从低比特缓存中读出 K/V，将其完整反量化为中间 FP16 张量，再把这些中间张量送入标准注意力计算。该过程功能正确，但会引入额外中间张量物化与高带宽显存读写开销。本文在 INT8 基准路径上采用 Triton 融合解码核函数，用同一注意力算法减少这些中间物化。
