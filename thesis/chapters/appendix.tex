...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lll}
    ...
    位宽 / 范围 & $n, q_{\max}$ & INT8 为 8 / 127，INT4 为 4 / 7 \\
...
\textbf{Qwen2.5-14B 单侧 PPL 补充。}
正文第~\ref{sec:exp-kv-sensitivity}~节引用的 14B 补充读数采用 32K prefix PPL 口径。
FP16 为 4.685，full \texttt{INT4} 为 5.040，\texttt{K16V4} 为 4.709，\texttt{K4V16} 为 4.813。
按 $(5.040 - x)/(5.040 - 4.685)$ 计算，\texttt{K16V4} 恢复约 93\%，\texttt{K4V16} 恢复约 64\%。
该补充读数用于校验 14B 上的方向一致性，按单模型内部对照解释。
...
并给出第~\ref{subsec:ch4-disc-kl-mse}~节正文读数的 7B KL/MSE 数据溯源。
...
表~\ref{tab:app-7b-kl-mse} 是第~\ref{subsec:ch4-disc-kl-mse}~节正文读数的溯源表。
