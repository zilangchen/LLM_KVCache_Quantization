...
表~\ref{tab:app-sig-quality} 合并列出了 INT8 路径在质量指标
（PPL/Needle/LongBench/RULER，$n=5$ 种子）与推理延迟
（TPOT，$n=8$ 种子）上的审计统计结果。
PPL 与 Needle 行对应主配置；LongBench 与 RULER 行使用主配置及补充配置的合并读数，
...
\begin{table}[!htbp]
  \centering
  \caption{INT8 质量与延迟审计统计（32K 上下文）}
  ...
  \begin{tabular}{llrrrrrr}
    ...
    \midrule
    \multirow{4}{*}{1.5B}
      & PPL$\downarrow$             & 8.9316 & 8.9497 & $-0.20$ & 0.0909 & 0.0974 & 否 \\
  ...
  \vspace{2pt}
  {\footnotesize 注：质量指标的差异均未达到 BH-FDR 校正后的统计显著性；三种模型上的 TPOT 降低均达到显著性（$q < 0.05$）。1.5B Needle 行报告主配置（5 种子均 100\%）；LongBench/RULER 行为主配置与补充配置的合并审计读数，其中 RULER 退化主要受补充配置影响。}
\end{table}

\phantomsection
\paragraph{上下文长度质量趋势图。}
