...
$N\in\{16, 32\}$。校准样本数量敏感性检查
（覆盖 $s\in\{16, 64, 256\}$）确认
...
图~\ref{fig:app-needle-depth-grid} 与图~\ref{fig:app-needle-exact}
补充展示 Needle 任务的深度位置热力图和精确匹配曲线。
二者服务于正文第~\ref{subsec:ch4-int4-cliff}~节的 cliff pattern：
它们说明对称 INT4 的退化主要在长上下文与深层 needle 位置上被放大，
并为附录第~\ref{sec:app-sqnr-derivation}~节的噪声--长度推导提供可视化背景。
...
本节只提供 Qwen2.5-1.5B-Instruct 上的系统补充图。
图~\ref{fig:app-tpot-gain}、图~\ref{fig:app-throughput-dashboard}
和图~\ref{fig:app-memory-dashboard} 分别对应 TPOT、吞吐和显存扩展读数；
正文部署结论仍以第~\ref{sec:exp-deployment}~节的跨模型表格为准。
...
\begin{figure}[!htbp]
  ...
  面板（c）展示 prefill 吞吐量。统一图例和坐标轴用于观察 batch 扩展中的并发收益与单序列代价。}
...
\begin{table}[!htbp]
  ...
  \vspace{2pt}
  {\footnotesize 注：（a）INT8-Canonical 与 INT4 融合模式均可支持 batch=32，INT8-Baseline 在 batch=32 时出现中间张量显存不足；融合实现将可服务 batch 从 24 提升到 32。（b）KV 压缩比约 3.76$\times$ 恒定；INT4-RoleAlign 的 TPOT 随 batch 增长，说明该实现仍有并发反量化摊销开销。batch=16 时其 KV Cache 为 968~MB（FP16 为 3640~MB），显示主要收益来自容量提升。}
\end{table}
...
表~\ref{tab:app-eng066} 是一组历史诊断读数，用于确认对称 INT4 失稳并非仅由
...
% ============================================================
\section{INT4 量化噪声与序列长度交互效应的近似机制分析}
\label{sec:app-sqnr-derivation}
% ============================================================

本节提供正文第~\ref{subsec:exp-int4-honest}~节中量化步长与序列长度交互效应的近似分析。
...
三种粒度下 FP16、INT4-RoleAlign 和 KIVI-style INT4 的 PPL 对比。
...
本节仅对正文第~\ref{sec:ch3-deployment}~节提及的 Triton 融合解码路径做实现族说明，
其职责是给出不同变体的语义边界与复现角色，而不在附录中额外引入新的速度结论。
当前实现族包括五类：INT8 融合解码主线实现、INT4 非对称融合核原型、
带自动调优的 INT4 迭代变体、显式兼容 GQA 头映射的 INT4 变体，
以及外部推理库适配层。这里按语义角色区分实现族，不暴露具体文件名或内部实现代号。
