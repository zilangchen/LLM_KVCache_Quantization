...
本节只承担复现审计映射职责。
表~\ref{tab:ch3-kv-modes} 给出正文量化路径名称与代码配置标识之间的对应关系；
\code{kv\_mode} 仅作为复现实验时的配置入口，正文论证只使用左列名称。

\begin{table}[!htbp]
  \centering
  \caption{正文量化路径与代码配置标识的对应关系}
...
其中 $N=128$ 表示正文口径下的标准校准规模；冻结复现入口使用
...
并为附录第~\ref{sec:app-int4-mechanism-boundary}~节中的噪声--长度推导提供可视化背景。
...
包括单序列 TPOT、吞吐/显存扩展图，以及 batch 维度的容量边界。
这些材料只服务于第~\ref{sec:exp-deployment}~节的部署边界审计；
硬件、backend、batch size 与 seq\_len 条件均不应外推为通用速度结论。
...
\begin{figure}[!htbp]
  ...
  \caption{各量化模式相对 FP16 基线的 TPOT 变化（Qwen2.5-1.5B-Instruct，NVIDIA H20，batch=1）。
...
\begin{figure}[!htbp]
  ...
  \caption{吞吐扩展性三联图（Qwen2.5-1.5B-Instruct，NVIDIA H20，seq\_len=8192，gen\_len=128）。
  面板（a）展示总吞吐量，面板（b）展示单序列吞吐量，
  面板（c）展示 prefill 吞吐量。统一图例和坐标轴用于观察当前 backend 组合下 batch 扩展中的并发收益与单序列代价。}
...
\begin{figure}[!htbp]
  ...
  \caption{显存扩展性双联图（Qwen2.5-1.5B-Instruct，NVIDIA H20，seq\_len=8192）。
...
\begin{table}[!htbp]
  \centering
  \caption{批处理扩展综合数据（Qwen2.5-1.5B-Instruct，NVIDIA H20，seq\_len=8192，独占 GPU）}
  \label{tab:app-batch}
  \small
  \resizebox{\textwidth}{!}{%
  \begin{tabular}{lccc}
  ...
  \vspace{2pt}
  {\footnotesize 注：（a）在 seq\_len=8192、NVIDIA H20 与当前 backend 组合下，INT8-Canonical 与 INT4 融合模式均可支持已测 batch=32；INT8-Baseline 在 batch=32 时出现中间张量显存不足，因此容量边界从 24 移至 32 只相对于该 baseline 解释。（b）KV 压缩比约 3.76$\times$ 恒定；INT4-RoleAlign 的 TPOT 随 batch 增长，说明该实现仍有并发反量化摊销开销。batch=16 时其 KV Cache 为 968~MB（FP16 为 3640~MB），主要收益体现在容量边界而非速度主结论。}
\end{table}
...
本节按三层组织 INT4 相关补充材料：首先用 scale 精度敏感性读数排除
“scale dtype 单点失误”解释；随后保留噪声--长度交互的近似推导，给出
32K 场景下 INT4 cliff 的机制直觉；最后用 chunk size 消融界定评估粒度和
streaming-like 协议边界。第三部分只限定协议适用范围，不引入新的主机制结论。
