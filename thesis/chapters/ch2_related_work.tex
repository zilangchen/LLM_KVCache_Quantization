...
\hat{x} = (x_q-z)\cdot s.
  \label{eq:asym_quant}
\end{equation}
当张量均值偏离零点，或不同维度的统计差异较大时，对称网格会把一部分整数等级留给样本很少落入的区间。非对称量化通过 zero-point 移动实数值与整数编码的对齐位置，使可表示区间贴近非零中心分布，在相同存储位宽下减少单侧范围浪费。这一参数入口也为后文 K/V 角色诊断提供基础。Key 侧主要暴露通道尺度差异，Value 侧主要暴露沿序列变化的动态范围，两类差异可以由各自的零点与缩放因子刻画，而不必压进一组共享参数。

量化轴决定缩放因子绑定在哪个维度。逐 channel 量化沿通道方向设置参数，更适合吸收通道间尺度差异。逐 token 量化沿时序方向设置参数，更适合刻画位置相关的动态范围变化。对 KV Cache 而言，轴向选择会进入功能角色。Key 参与 $\bq\bK^\top$，先影响 attention logits 及其相对次序，再经 softmax 转为权重分布；Value 在这些权重下被加权聚合。后文会同时说明使用多少 bit、bit 落在 K 还是 V 一侧，以及沿通道还是沿 token 分配。这样可以更清楚地呈现位宽、功能角色与量化粒度之间的关系。
