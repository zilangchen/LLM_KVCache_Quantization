...
M_{KV} = 2 \times L \times H_{kv} \times d_k \times S \times b .
  \label{eq:kv_mem}
\end{equation}
其中，系数 2 来自 Key 与 Value 两份缓存。这里默认每头满足 $d_v=d_k$。$L$ 表示层数，$H_{kv}$ 表示每层实际参与缓存的 KV 头数，$d_k$ 表示头维度，$S$ 表示序列长度，$b$ 表示单元素字节数，例如 FP16、INT8 与 INT4 分别对应 2、1 和 0.5。结合上一节的 MHA、MQA 与 GQA 结构可知，缓存规模由共享后的 $H_{kv}$ 决定，而不是由查询头数 $H_q$ 决定。当模型从标准 MHA 过渡到 MQA 或 GQA 时，$H_{kv}$ 的降低会直接减少缓存存储需求，这也是不同模型在长上下文部署成本上出现显著差异的重要结构原因之一。
...
\hat{x} = (x_q-z)\cdot s.
  \label{eq:asym_quant}
\end{equation}
当张量均值偏离零点，或不同维度的统计差异较大时，对称网格会把一部分整数等级花在样本很少落入的区间。非对称量化通过 zero-point 移动实数值与整数编码的对齐位置，使可表示区间贴近非零中心分布，在相同存储位宽下减少单侧范围浪费。这个参数入口也对应后文的 K/V 角色诊断。Key 侧主要暴露通道尺度差异，Value 侧主要暴露沿序列变化的动态范围，两类差异可以分别由独立的零点与缩放因子刻画。

量化轴决定缩放因子绑定在哪个维度。逐通道量化沿通道方向设置参数，更适合吸收通道间尺度差异。逐 token 量化沿时序方向设置参数，更适合刻画位置相关的动态范围变化。

对 KV Cache 来说，轴向选择会进入功能角色。Key 参与 $\bq\bK^\top$，先影响 attention logits 及其相对次序，再经 softmax 转为权重分布。Value 在这些权重下被加权聚合。后文讨论存储位宽时，会同时说明 bit 落在 K 还是 V 一侧，以及它沿通道还是沿 token 分配。
