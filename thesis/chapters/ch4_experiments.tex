...
如果把第 4.4 节的 allocator 结果理解成“本文方法需要在所有区间持续高于 heuristic”，就会误读这组适用区间结果。更准确的观察是：在多个模型与预算带内，position-based heuristic 与 behavior-guided 策略的结果非常接近，甚至在选中层集上出现局部重合。这一观察本身已经构成本文结论的一部分：heuristic 是\textbf{强基线}，而且这一点必须被正面承认。

heuristic 的强来自结构本身：在某些适用区间下，关键层集中在一个很窄的位置区间，因此“保护哪几层”更接近窄选择空间，而非高自由度优化问题。此时位置启发式能够逼近行为引导策略，说明结构本身已经把可行解压窄了。换言之，强启发式基线并不削弱本文的结构读法，反而提示某些适用区间确实具有狭窄而稳定的可行层集。
