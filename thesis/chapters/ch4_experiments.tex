...
\begin{table}[H]
  ...
  \begin{tabular}{lcccc}
    ...
    \midrule
    Qwen2.5-1.5B & 2 & 100\% & 0\%  & 阶跃崩塌；PPL $9.31 \rightarrow 19.5$ \\
    Qwen2.5-7B   & 4 & 100\% & 0\%  & 阶跃崩塌；PPL $7.14 \rightarrow 85.5$ \\
    LLaMA-3.1-8B & 8 & 100\% & 98\% & 架构例外；PPL $6.73 \rightarrow 6.97$ \\
    \bottomrule
  \end{tabular}
...
\begin{table}[H]
  ...
  \begin{minipage}{0.94\linewidth}
    \footnotesize
    \emph{注} Panel A 与 Panel B 共享生成长度 64 协议（10 次运行），与表~\ref{tab:ch4-tpot-4k}~的入口层（生成长度 128）不同，因此同模型 4K 处的 $\Delta T$ 与从表~\ref{tab:ch4-tpot-4k}~推出的差值略有不同。Panel A 与 Panel B 分别对应第 4.5.2 节与第 4.5.3 节。这里的“性能交叉边界”是经验读数，并非理论闭式模型；8B vs 14B 的同 $H_{kv}=8$ 控制对比不单独列成表，而在第 4.5.3 节正文作为补充段落报告。4K 处 $|\Delta T|<2$\,ms 的读数统一标记为 n.s.，不作为交叉点读取。
  \end{minipage}
  }
\end{table}
