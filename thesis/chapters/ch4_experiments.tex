...
表~\ref{tab:ch4-kv-ppl} 与表~\ref{tab:ch4-kv-multitask} 已经把低比特失效定位到 Key 侧，因此本小节不再在对称 \texttt{INT4} 参数空间里继续比较。这里进入 \texttt{per-channel K + per-token V} 的同一非对称格式，并只比较两种实例化来源：离线 role-aware calibration 的 \texttt{INT4-RoleAlign}，以及运行时 \texttt{absmax/min} 的 \texttt{KIVI-style}。二者的设计差异已在第三章第~\ref{subsec:ch3-rolealign-vs-kivi}~节固定，本小节只读取表~\ref{tab:ch4-rolealign-kivi} 的同格式结果。
...
校准来源的差异仍然有方法层价值，但表~\ref{tab:ch4-rolealign-kivi} 未显示稳定、可外推的质量分化。\texttt{KIVI-style} 在运行时根据 \texttt{absmax/min} 更新 scale；\texttt{RoleAlign} 则把 K-path 与 V-path 的分路校准结果固化为离线产物，并把同一产物继续交给第 4.4 节的行为引导分配器。因此，本小节读取的是“同格式恢复 + 离线产物组织”的组合，不将其读成跨模型无条件优越。
