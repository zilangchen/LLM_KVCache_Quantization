...
表~\ref{tab:ch4-model-matrix} 同时承担本章后续典型剖面的模型锚定：Qwen2.5-1.5B-Instruct 作为 INT8 基准路径与官方 LongBench 真实数据对照的锚点模型，Qwen2.5-3B-Instruct 作为小模型分配策略剖面，Qwen2.5-7B-Instruct 作为同族补充对照，LLaMA-3.1-8B-Instruct 作为中等规模与 $H_{kv}=8$ 的结构对照，Mistral-7B-Instruct-v0.3 作为 Mistral 家族的分配策略剖面，而 Qwen2.5-14B-Instruct 则作为大模型分配策略剖面。这里的“实验角色”只表示后续证据承载功能，而不预告任何结果。

所有实验均在单张 NVIDIA H20 GPU（98\,GB HBM）上完成。软件环境统一为 Python 3.12、PyTorch 2.8.0（CUDA 12.8）、Transformers 与 Triton；完整依赖清单随复现材料发布。除特别说明外，生成式质量评测与吞吐量测试统一采用贪婪设置\footnote{除特别说明外，生成式任务采用贪婪解码：\texttt{temperature=0.0}、\texttt{top\_p=1.0}、\texttt{top\_k=0}。}，以消除采样随机性带来的额外方差；PPL 则采用固定输入序列上的 likelihood 累计，不涉及采样解码。
...
语言建模质量采用 PPL 评测。为保证不同量化路径在相同时间语义下比较，PPL 使用统一的 32K 上下文协议，并将 \texttt{chunk\_size} 固定为 128；这一设置决定了交叉熵累计时每个位置所能访问的历史窗口，因此不能被视为可随意替换的实现细节。对显存充足的模型，PPL 在完整测试集协议下报告；对 14B 模型，由于 H20 显存约束，仅在测试集前固定前缀上评测，因此其绝对值不与其他模型做跨协议直接比较。
...
表~\ref{tab:ch4-method-config} 固定了承重比较对象。在校准理念维度上，percentile 路径只利用数值分布的截断信息，离线行为引导路径则以注意力行为中的分布偏移作为可操作代理，而 INT4-RoleAlign 进一步将这一原则落实为角色感知的离线校准；KIVI-style 的 \texttt{absmax/min} 则在运行时根据统计量更新。量化轴维度用于区分对称 per-group 与非对称 per-channel K + per-token V 两类格式；在这个维度上，INT4-RoleAlign 与 KIVI-style 共享相同的缓存格式，因此两者的差异被明确限制在校准理念上，而不是格式本身。Scale 时间语义则进一步区分离线固化、逐 token 动态更新以及 K/V 混合时间语义三类部署方式。

这里还需给出两条边界声明。第一，MixedKV 仅用于第 4.3.2 节的 K/V 角色敏感性诊断，不参与主线方法比较。第二，逐头温度校正仅作为附录中的补充诊断变体，本章主线配置不使用该组件。这样安排的目的，是让第四章后续所有比较都回到同一张设计坐标系中，而不是在不同格式、不同校准理念和不同运行时语义之间交叉混写。
...
本章还采用一个与一般 LLM 评测略有不同的读法：PPL 在固定输入序列、固定 tokenization、固定窗口与固定 loss mask 下是确定性 likelihood 指标，因此其可靠性主要由协议一致性与效应量本身承担，而不是由 $p$ 值承担。PPL 读数值幅度，不读统计显著性。
...
最后，后文所有主表、主图与典型剖面，都应在上述协议之下理解：不同协议之间的绝对分数不做直接横向比较；官方 LongBench 真实数据对照只承担第 4.2.2 节的一致性检验职责；完整 LongBench-style 聚合、统计检验汇总与上下文长度质量趋势保留在附录第~\ref{sec:app-longbench-full}~节，仅作补充审计，不进入正文主证据矩阵；部署效率结论严格限定于当前 H20 硬件、固定软件栈与当前后端组合；正文的主结论只由上述统一协议下的主结果承担。后续各节据此只回答两件事：这些证据是否成立，以及成立到哪里为止。
