...
\begin{table}[H]
  ...
  \begin{minipage}{0.94\linewidth}
    \footnotesize
    注：本表只汇总对称 \texttt{INT4} 的最小现象锚点。Qwen2.5-1.5B/7B 出现“100\%\,$\rightarrow$\,0\%”的检索阶跃；LLaMA-3.1-8B（$H_{kv}=8$）作为架构例外保留 98\% Needle。
  \end{minipage}
  }
\end{table}
...
\begin{table}[H]
  ...
  \begin{minipage}{0.94\linewidth}
    \footnotesize
    注：Qwen2.5-14B 当前仅提供 RoleAlign 结果，不参与配对差异判断。
  \end{minipage}
  }
\end{table}
...
\begin{table}[H]
  ...
  \begin{minipage}{0.94\linewidth}
    \footnotesize
    注：粗体标注同模型 INT4 后端路径中的最快项。该表作为第 4.5.1 节的入口层对照，不用于宣称 4K 已形成加速收益；收益边界需结合后续长序列读数与性能交叉边界共同解释。
  \end{minipage}
  }
\end{table}
