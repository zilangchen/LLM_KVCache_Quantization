\begin{table}[!htbp]
  ...
  \begin{tabular}{lcccc}
    \toprule
    Task & FP16 & \textbf{INT8-Canonical} & INT4-RoleAlign & KIVI-style \\
    \midrule
    ...
    \bottomrule
  \end{tabular}
  \caption{INT8 Canonical Path 保真度（Qwen2.5-1.5B-Instruct）。每单元格为分数，括号内为相对 FP16 的 $\Delta$。\textbf{INT8-Canonical} 的 mean $\Delta$ 加粗展示，用于支持本章的基准路径保真结论。}
  \label{tab:t1-int8-canonical}
  \par\smallskip\noindent{\footnotesize INT4-RoleAlign 与 KIVI-style 在此表仅为同一评测口径下的格式参考值，完整跨模型对比见表~\ref{tab:t2-int4-kivi}。}
\end{table}
