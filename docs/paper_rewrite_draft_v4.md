...
>
> This file is a later-stage rewrite draft. It keeps the new paper line from `v1` while tightening the writing toward a conference-style manuscript.
>
...
KV cache compression is increasingly necessary for long-context large language model inference, yet its quality degradation is not fully captured by numerical reconstruction error alone. We argue that **attention behavior** is the more appropriate object to analyze and preserve, and we use it to organize calibration, mixed-precision allocation, and downstream budget proposal for KV cache quantization. We first validate this view through an **INT8 canonical validated instance**, which provides the cleanest end-to-end path from behavior-aligned calibration to deployable runtime integration without claiming universal superiority over standard numeric objectives. We then study mixed-precision KV allocation across multiple model families and scales and find that the most defensible empirical reading is **regime-dependent**: allocator quality varies with family, scale, and task, positional heuristics remain strong baselines, and no single fixed protection budget transfers cleanly across all models. To reduce brittle fixed-budget sweeps, we introduce **auto-k** as a profile-aware budget proposer derived from behavior-guided sensitivity profiles. Auto-k is best understood as a strong extension rather than the theoretical center of the paper: it is competitive across models, most clearly positive on Mistral-7B, and especially informative when contrasted with the early-layer bottleneck observed on 3B. Overall, the paper contributes a framework and a regime-based reading of KV cache quantization rather than a universal winner story.
...
Quantization is a natural response to this bottleneck, but the usual analysis remains incomplete. Many pipelines are organized around numerical proxies such as reconstruction error, clipping loss, or mean squared error. These quantities are useful, but they do not fully describe what KV cache quantization disrupts at inference time. Keys and Values enter attention score computation directly, so seemingly mild tensor perturbations can become much larger behavioral changes once they pass through logits, softmax normalization, and decoding. The relevant question is therefore not only whether tensors remain numerically close, but whether the model's **attention behavior** remains intact.

This observation motivates the paper's main thesis: **behavior** should be treated as the unified analysis and design principle for KV cache quantization. Under this view, calibration is not merely the search for scales and clipping rules, and mixed-precision allocation is not merely the search for a convenient protection budget. Both are decisions about how behavioral distortion enters the system and where that distortion becomes most harmful.

We operationalize this view in two layers. The first is **behavior-aligned calibration**, which produces deployable calibration artifacts from behavior-sensitive signals and is most cleanly validated in an INT8 setting. The second is **behavior-guided allocation**, which consumes behavior-derived profiles to decide where higher precision should be preserved under a constrained KV budget. Calibration determines *how* a layer is quantized; allocation determines *where* limited precision should be spent.

The role of INT8 in this paper is deliberately narrow and precise. We use an **INT8 canonical validated instance** as the cleanest end-to-end path for validating the framework: calibration artifacts can be constructed offline, consumed by the runtime, and integrated with fused decode execution in a setting that is aggressive enough to matter but controlled enough to attribute. INT8 is therefore not the paper's frontier low-bit target. It is the most auditable setting in which the framework can be validated as a real deployment path, and it should not be read as proving that behavior-based calibration universally outperforms MSE.
...
This empirical structure changes how method design should be interpreted. Fixed protection budgets are often too brittle to transfer across models, while positional heuristics are far stronger baselines than a simple behavior-superiority narrative would suggest. In this setting, **auto-k** matters because it converts behavior-derived profiles into reusable budget suggestions. We therefore present auto-k as a **profile-aware budget proposer** and a strong downstream extension of the framework, not as the theory center of the paper.

Our contributions are fourfold. First, we propose a **behavior-centric framework** for KV cache quantization that links calibration and allocation through a common analysis object. Second, we establish an **INT8 canonical validated instance** as the cleanest end-to-end validation route for this framework. Third, we argue that mixed-precision allocator outcomes are best interpreted as **regime-dependent**, not universally optimal, and we explicitly reposition heuristic baselines as serious comparators. Fourth, we introduce **auto-k** as a profile-aware extension supported by meaningful but bounded evidence, with its strongest explicit support currently concentrated in Mistral and with its interpretation constrained by provenance.

Figure 1 summarizes this reading rule before the paper enters technical detail. Rather than visualizing three parallel contributions, it should present one causal chain from analysis object to framework validation to allocator reading.
...
On the format axis, the key reference point is **KIVI** [KIVI], which studies KV-cache distributions and argues for an asymmetric design in which Keys are quantized per channel while Values are quantized per token. This is foundational because it establishes that low-bit KV quantization should respect the structural differences between K and V rather than quantize both sides symmetrically. More recent work such as **AsymKV** [AsymKV] pushes this line further with more aggressive low-bit settings and layer-wise asymmetric configurations, again emphasizing that Key-side perturbations are often more consequential than Value-side ones.

Our positioning relative to this line is deliberately narrow. We do not claim to have invented a wholly new low-bit K/V format outside the broad asymmetric design family established by KIVI and expanded by AsymKV. Nor do we claim decisive numerical victory over KIVI-style baselines. The contribution here is interpretive and organizational: we use a **behavior-centric lens** to explain why K/V asymmetry matters for attention preservation, and we show how the resulting behavior-derived profile can support decisions beyond the immediate quantizer configuration.

### 2.2 Calibration signal: from numerical proxies to behavior-sensitive objectives

On the calibration axis, most pipelines still rely on numerical proxies such as MSE, percentile clipping, or related tensor-reconstruction criteria. These objectives are natural and often sufficient in mild regimes, but they do not directly represent the object that ultimately matters for decoding quality: whether the attention computation itself is distorted in a way that changes generation behavior.

The behavior-oriented perspective asks a different question. Instead of asking only whether the quantized tensors remain close in Euclidean terms, it asks whether the *functional role* of Keys and Values inside attention remains stable. In the present work, this perspective is instantiated as **behavior-aligned calibration**. The current paper, however, is intentionally more conservative than a simple “behavior beats MSE” narrative. The present INT8 evidence supports the role of behavior as a meaningful design object and a stable source of calibration artifacts, but it does not support a universal superiority claim over all numeric proxies.

### 2.3 Allocation policy: mixed precision and layer-wise budgeting

The third axis concerns **allocation**. Once a single uniform precision is insufficient, the problem becomes one of deciding where the limited high-precision budget should be spent. Here the closest prior lines are mixed-precision and layer-wise allocation methods. **KVTuner** [KVTuner] treats layer-wise sensitivity as a search problem and performs offline exploration of hardware-friendly mixed-precision KV configurations. **KVmix** [KVmix] pushes further toward gradient-based importance analysis and explicitly frames mixed-precision KV quantization as a layer-importance problem under quality-efficiency trade-offs. Together, these papers establish that unequal layer importance is real and that layer-wise budget assignment is a central part of the modern KV design space.

Our paper is aligned with that trend but differs in emphasis. We do not primarily contribute a stronger optimizer for per-layer bit assignment. Instead, we contribute a **framework-level connection**: the same behavior-derived profile that supports calibration can also guide allocation. This shifts the interpretation of allocation from a purely heuristic search problem toward a profile-driven policy problem. At the same time, we explicitly avoid claiming that the resulting profile yields a universal winner policy.

### 2.4 Adjacent lines: attention-driven control and token-level memory selection

A conceptually adjacent line studies token-level KV retention or eviction rather than quantization. **H2O** [H2O] identifies heavy-hitter tokens and frames KV eviction as a structured token-retention problem. **SnapKV** [SnapKV] uses an observation window to identify prompt features that attention heads repeatedly focus on and compresses KV caches by selecting important KV positions. More recent attention-driven control methods such as **AhaKV** [AhaKV] refine these ideas by addressing bias in accumulated attention scores and incorporating more holistic attention information. Related head-specialization work such as **DuoAttention** [DuoAttention] also shows that long-context efficiency can depend on structured distinctions inside the attention mechanism itself.

These works do not solve the same problem as KV quantization, but they are highly relevant because they reinforce the broader premise that **attention-derived signals can be operationally useful compression signals**. We therefore treat them as conceptually adjacent rather than directly competitive. Eviction decides *what to keep*; quantization decides *how precisely to keep it*. Their budgets differ, but the guiding intuition overlaps.

### 2.5 Positioning of this paper

Table 1 should make the paper's position explicit before the method section begins. The goal is not to present a winner matrix, but to show which design axis each prior line primarily addresses and where the present paper actually sits.

> **Table placeholder (Table 1): Prior Work Positioning Matrix**
>
> Columns: `Primary axis`, `Representative methods`, `What they solve well`, `What they do not claim`, `Relation to this paper`. Rows should cover format-oriented work such as KIVI and AsymKV, calibration-oriented work, layer-wise allocation methods such as KVTuner and KVmix, and adjacent attention-driven memory-selection methods such as H2O, SnapKV, AhaKV, and DuoAttention.
...
### 4.3 Cross-model regime reading
...
Table 3 should introduce this view before the paper enters model-specific discussion. Its job is not to rank policies, but to summarize what kind of allocator behavior each model reveals and what kind of claim is safe to make in each case. Figure 3 then provides the complementary visual: the paper is not discovering a single optimum, but mapping a small set of qualitatively distinct operating regimes.
...
### 4.4 7B: the strongest regime-defining finding
...
The stable reading is that 7B provides the cleanest evidence that allocation should be read through operating regimes rather than through one transferable fixed rule. The paper should not over-interpret this section as establishing a law that must persist across all larger or smaller models.

### 4.5 8B: where the old fixed-k story breaks
...
The stable reading is that auto-k becomes a competitive extension here: it is clearly stronger than weaker fixed-budget or heuristic families in the current compare set, but it does not take first place. The unsafe reading would be that 8B already proves auto-k is the best policy or that the best budget simply shifts upward with model scale.

### 4.6 14B: broad high-quality band and strong baselines

The 14B setting strengthens another message: **heuristic and uniform baselines remain serious competitors**. This is not an embarrassment to hide. It is part of the paper's strongest reading. When a larger model exhibits a broad high-quality band across several policies, the allocator problem should be interpreted as a constrained regime problem rather than as a contest against weak baselines.

The stable reading is therefore top-tier coexistence rather than decisive superiority. The unsafe reading would be that the 14B result somehow restores a universal scale law or confirms that one policy family has clearly dominated the design space.

### 4.7 Mistral: the strongest explicit positive case for auto-k

Mistral provides the cleanest explicit positive evidence for auto-k in the current compare set. It is the model on which auto-k most clearly reaches the top of the observed range while remaining compatible with the broader regime-based reading. This is why Mistral matters in the paper: it turns auto-k from a conceptual extension into an empirically supported one.

The stable reading is therefore not “auto-k wins everywhere,” but “auto-k has now moved from conceptual extension to empirically supported extension, with its cleanest explicit win concentrated in Mistral.” The unsafe reading would be to generalize that result into cross-family universal superiority.

### 4.8 Heuristic is a strong baseline, not a strawman
...
### 4.9 Auto-k is a strong extension, not the theory center
...
### 4.10 The 3B anomaly: early-layer bottleneck and first-layer rescue
...
Figure 4 should be introduced as the paper's anomaly figure rather than as another policy comparison chart. Its role is to make the first-layer rescue phenomenon visually unavoidable while keeping the interpretation bounded.
...
### 4.11 Provenance and evidence governance
...
The rewritten paper should be explicit about what it has established and what it has not. The current evidence supports treating **behavior** as a useful unified analysis and design principle for KV cache quantization. It also supports the INT8 path as a clean canonical validation route for the framework and supports reading allocator outcomes through **regime-dependent** structure rather than universal laws.
...
These limitations point naturally to the next set of extensions. One natural next step is **K/V-asymmetric allocation**, which would test whether the observed imbalance between Key-side and Value-side fragility can be translated into a role-specific bit-budget policy. A second is **quality-cost Pareto analysis**, which is necessary if the allocator line is to mature from a quality-only narrative into a true budget-allocation method that reasons jointly about latency, memory, and quality. A third is **prompt-adaptive policy selection**, where the current static policy artifact could be complemented by a prompt- or task-conditioned selector.
...
This paper argues that KV cache quantization can be fruitfully organized around **attention behavior** rather than around numerical reconstruction error alone. From that starting point, it presents a **behavior-centric framework** that links calibration and allocation through behavior-derived artifacts and policy decisions. The cleanest validation of this framework is provided by an **INT8 canonical validated instance**, which demonstrates that behavior-grounded calibration can be made auditable and deployable without forcing an unwarranted superiority claim over standard numeric objectives.
...
The contribution of the rewritten paper is therefore not a single dominating method. It is a more durable structure for the problem: behavior as the central design object, INT8 as the canonical validation route, regime-dependent allocation as the correct empirical reading, and auto-k as a bounded but valuable extension. This framing better matches the current evidence boundary by emphasizing framework validity, regime-dependent interpretation, and bounded extension claims rather than universal superiority.

## Author-Side Revision Notes (remove before submission)
