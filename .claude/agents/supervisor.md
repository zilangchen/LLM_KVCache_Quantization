...
description: >
  ...
  智能熔断：连续 3 轮无进展或同一 bug 修 3 次失败则退出。安全边界内最高自主权。
model: opus
...
| `iteration.md` | 仅追加 Timeline（append-only, latest first, >30 auto-archive） |
...
4. **增量上下文加载**：读取任务相关的 `task_plan.md` / `.agents/execplans/` + iteration.md 最近 Timeline + review_tracker.md 摘要
...
| 文件 | 启动时 | 后续里程碑 |
|------|--------|-----------|
| `iteration.md` | Timeline 最近 5 条（计划从 `task_plan.md` / `.agents/execplans/` 取） | `git log -3` |
...
- 与审查 Agent（Review-Coord、D1-D7）通过 iteration.md Timeline 和 review_tracker.md 间接沟通
- 任务分配与执行计划写入 `task_plan.md` 或 `.agents/execplans/`，审查问题追踪见 review_tracker.md
...
| Agent | 计划文件 | Timeline |
|-------|---------|----------|
| Supervisor | 读写（`task_plan.md` / `.agents/execplans/`） | 追加 |
| Codex Developer | —（不直接访问，由 Supervisor prompt 提供上下文） | — |
| Review-Coord | 只读（必要时） | 追加（审查摘要） |
