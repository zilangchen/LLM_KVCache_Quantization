...
增量审查时，根据变更文件类型选择相关维度，减少不必要的 Agent 调用：

| 变更类型 | 必选维度 | 可选维度 |
|---------|---------|---------|
| `src/quant/` `src/cache/` `src/kernels/` | D1(数值) D5(边界) | D2(静默) D4(契约) |
| `src/engine/` | D4(契约) D2(静默) D5(边界) | D1(数值) |
| `scripts/eval_*` `scripts/aggregate_*` | D1(数值) D2(静默) | D6(测试) |
| `scripts/run_*` `scripts/calibrate_*` | D2(静默) D3(安全) | D5(边界) |
| `tests/` `configs/` | D6(测试) D4(契约) | — |
| 混合/大范围变更 | 全部 7 个 | — |

全量深度审查模式始终 spawn 全部 7 个 Agent（不裁剪）。
...
### Step 2.5: Codex 交叉审查（必须）

在 D1-D7 并行审查的同时或之后，**必须**调用 Codex (GPT-5.4) 进行交叉审查：

**CLI 模式（增量审查时）**：
```bash
codex review --base $last_reviewed_commit "审查以下代码变更，重点关注：1)数值精度 2)边界条件 3)静默失败 4)接口契约变化 5)安全漏洞。按严重性分级(CRITICAL/HIGH/MEDIUM/LOW)输出，每条包含文件路径和行号。"
```

**MCP 模式（全量审查或需要自定义指令时）**：
```
mcp__codex__codex(
  prompt: "审查以下文件：<文件列表>。重点关注：<本模块关注点>。按严重性分级输出。",
  sandbox: "read-only",
  cwd: "/Users/chenzilang/Desktop/LLM_KVCache_Quantization"
)
```

**异常处理**：如果 Codex 调用失败（超时/认证过期/CLI 错误），**必须重试一次**。仍失败则记录 `⚠️ Codex 交叉审查失败：<失败原因>`，使用 D1-D7 结果继续，但在审查报告中**明确标注 Codex 审查缺失**。

**Codex 发现处理**：
- 解析 Codex 输出，按现有格式追加到审查结果
- Issue ID 使用对应模块的标准前缀（如 `QNT-`, `ENG-`）
- 在 issue 描述末尾追加 `[Codex/GPT-5.4]` 标注来源
- 严重性对齐现有四级分类

### Step 3: 汇聚结果

每个 Agent 返回后，收集其发现（包括 D1-D7 和 Codex 的结果）。然后：

1. **按严重性排序**：CRIT → HIGH → MED → LOW
2. **去重**：检查是否与 review_tracker.md 已有 issues 重复（跨来源去重：同一问题 Claude 和 Codex 都发现只保留一条，优先保留更详细的）
3. **来源标注**：确保每条发现标注来源（D1-D7 无需标注，Codex 发现标注 `[Codex/GPT-5.4]`）
4. **统计摘要**（见输出格式，分别统计 Claude 和 Codex 的发现数量）
5. **逐条展示**：每个发现的详细信息

### Step 4: 写入 + 校验 + 记录

新发现已由各专项 Agent 直接写入 review_tracker.md。协调员做最终校验、整理和记录：

1. `python scripts/review_tool.py stats` — 统计是否一致
2. 检查 Phase Gate 状态变化
3. **整理 review_tracker.md**：扫描 Open Issues 和 Phase Blockers 中是否有残留的 `[x]` 条目（developer 可能忘记移动）。如发现，将其移至 Resolved `<details>` 区域对应 section 下。确保 Open Issues 只包含 `[ ]` 条目、Phase Blockers 只包含 CRITICAL `[ ]` 条目。
4. `date '+%Y-%m-%d %H:%M'` — 获取真实时间
5. 追加 iteration.md Timeline 审查摘要：

```markdown
### <真实时间> | Review-Coord 审查报告
- Mode: 增量 (commit range) / 全量 (模块名)
- New findings: X (C CRIT, H HIGH, M MED, L LOW)
- Phase Gate: BLOCKED / CLEAR
- Deep review progress: M/10 modules, cycle #N
```
...
- **每轮 Agent 调用数量**：增量审查如果只涉及 2 个文件且只需 3 个维度，休眠可适当拉长
...
- 新发现: X (C CRIT, H HIGH, M MED, L LOW)
  - Claude D1-D7: Y 条 | Codex: Z 条 (或 "跳过: <原因>")
- Phase Gate: BLOCKED / CLEAR
