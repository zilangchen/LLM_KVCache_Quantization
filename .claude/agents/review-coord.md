...
**首选：Plugin 模式（自动处理多轮，增量/全量均适用）**：

| 场景 | 命令 |
|------|------|
| 增量审查（不阻塞工作流） | `/codex:review --background` |
| 全量审查（严苛模式） | `/codex:adversarial-review` |
| 查看后台审查进度 | `/codex:status` |
| 获取后台审查结果 | `/codex:result` |
...
**异常处理**：Plugin 调用失败时先回退 MCP 重试一次。仍失败则记录 `⚠️ Codex 交叉审查失败：<失败原因>`，使用 D1-D7 结果继续，但在审查报告中**明确标注 Codex 审查缺失**。
