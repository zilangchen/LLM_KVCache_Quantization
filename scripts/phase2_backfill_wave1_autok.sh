#!/bin/bash
# =============================================================================
# Phase 2.6 Backfill: Wave 1 (8B) auto-k only runner
# =============================================================================
# 仅补跑 Wave 1 缺失的 3 个 auto-k policies：
#   bakv_auto_cov70_max / bakv_auto_cov80_max / bakv_auto_cov90_max
#
# 用法：
#   CUDA_VISIBLE_DEVICES=0 bash scripts/phase2_backfill_wave1_autok.sh narrativeqa
#   CUDA_VISIBLE_DEVICES=1 bash scripts/phase2_backfill_wave1_autok.sh hotpotqa
#   CUDA_VISIBLE_DEVICES=2 bash scripts/phase2_backfill_wave1_autok.sh gov_report
# =============================================================================
set -euo pipefail
...
phase2_backfill_verify_wave1_task() {
    local out_dir="$1"
    local task="$2"
    shift 2
...
print(f"=== Wave 1 backfill task {task} 精确校验 ===")
...
echo "=== Wave 1 auto-k backfill task=$TASK @ $(date) ==="
...
echo "=== Wave 1 auto-k backfill task $TASK 日志校验 ==="
...
if [ "$traceback_cnt" -ne 0 ] || [ "$head_mismatch_cnt" -ne 0 ]; then
    echo "[Wave 1 auto-k backfill task $TASK] GATE FAIL" >&2
    exit 3
fi

phase2_backfill_verify_wave1_task "$OUT_DIR" "$TASK" "${RUN_NAMES[@]}"
echo "[Wave 1 auto-k backfill task $TASK] GATE PASS"
