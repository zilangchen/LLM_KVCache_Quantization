...
CALIB="artifacts/kv_calib_kl_qwen25_14b_int8.json"
OUT_DIR="artifacts/allocator/sweep_14b"
mkdir -p "$OUT_DIR"

if [ ! -f "$CALIB" ]; then
    echo "FATAL: 14B calib missing: $CALIB" >&2
    echo "  → 先跑 bash scripts/phase2_calibrate_14b.sh（内部使用 --calib_out）" >&2
    exit 2
fi

ALLOCATOR="python3 scripts/adaptive/behavior_aligned_allocator.py --calib $CALIB"

echo "=== Wave 4: 14B policy 生成 ==="
...
python3 - <<'PYEOF'
import json, os
d = "artifacts/allocator/sweep_14b"
for k in [1, 3, 5, 7]:
