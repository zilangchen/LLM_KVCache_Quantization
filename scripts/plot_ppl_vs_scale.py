...
bars_ra = ax.bar(x - width/2, ppl_rolealign, width,
                 label="INT4-RoleAlign", color="#3498db", edgecolor="white", linewidth=0.8, zorder=3)
bars_kv = ax.bar(x + width/2, ppl_kivi, width,
                 label="KIVI-style INT4", color="#e74c3c", edgecolor="white", linewidth=0.8, alpha=0.75, zorder=3)
...
for bar, val in zip(bars_kv, ppl_kivi):
    ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.3,
            f"{val}%", ha="center", va="bottom", fontsize=8.5, color="#c0392b")
...
ax.annotate("", xy=(2.4, 3.5), xytext=(0.0, 14.5),
            arrowprops=dict(arrowstyle="->, head_width=0.3", color="#95a5a6", lw=1.5, ls="--"))
