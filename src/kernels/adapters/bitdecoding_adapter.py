...
def decode_attn_bitdecoding(
    ...
    # Pad with last-token repeat (not zeros) to avoid per-token scale=0 → NaN
    # in BitDecoding's kvcache_pack_int.  Zero-padded tokens have zero scale,
    # which causes division by zero during per-token dequant inside the kernel.
    if padded_s > S:
        pad_len = padded_s - S
        k_last = k_bshd[:, S - 1:S, :, :].expand(B, pad_len, Hkv, D_full)
        v_last = v_bshd[:, S - 1:S, :, :].expand(B, pad_len, Hkv, D_full)
