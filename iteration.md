...
### 2026-03-18 19:43 | Closure Pack 启动 — 论文最小闭环 GPU 实验
- Goal: 执行 Closure Pack A1-A4，补齐 MixedKV LongBench/RULER 数据缺口
- Changed files: scripts/closure_gpu0.sh, scripts/closure_gpu1.sh (新建)
- GPU 0 任务链: A1(1.5B/8B LongBench) → A3(C6 INT8 RULER fix) → A2(1.5B/8B RULER)
- GPU 1 任务链: A1(7B/Mistral LongBench) → A2(7B/Mistral RULER) → A4(8B attention KL)
- 远端: 2×H20 全空闲, 23GB disk free, 校准文件已验证
- 结果目录: emnlp_postfix_v2/runs (A1/A2/A4), emnlp_c6_fix/runs (A3)
- Validation: 待 GPU 实验完成后聚合验证
- Risks: Mistral 模型可能需重下(HF_HUB_OFFLINE=1 若缓存不存在会失败); RULER 时间可能超预期
