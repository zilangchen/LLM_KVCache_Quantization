...
### Plan: Expansion Pack 高信号版 (5 Phases, ~14-17h GPU)
- **批准日期**：2026-03-19
- **前置条件**：Closure Pack Part A 完成, Phase 0 gate 通过
- **状态**：🟢 Phase 0 通过, 全部文件已创建, 待 GPU 执行
- **Phase 0**: ✅ 本地验证通过 (Table 3 无 "---", .tex 全部存在)
- **Phase 1**: B10 校准灵敏度 (1.5B + 7B, 6 calib × 3 eval, ~4h)
  - [ ] 6 calib JSONs (s16/s64/s256 × 1.5B/7B)
  - [ ] 6 YAML configs ✅ 已创建
  - [ ] PPL + Needle + LongBench 评测
- **Phase 2**: K/V 消融 LongBench (4 models × 3 ablation × 3 seeds = 36 runs, ~5h)
  - [ ] Mistral YAML K/V entries ✅ 已添加
  - [ ] 36 LongBench CSVs
- **Phase 3**: C6 修复 7B/8B RULER sanity (6 runs, ~1.5h)
- **Phase 4**: K/V 消融 RULER (3 models × 3 ablation × 3 seeds = 27 runs, ~4h)
- **Phase 5**: B9 Mistral 热力图 (~30 min)
- **脚本**: expansion_gpu0.sh, expansion_gpu1.sh, expansion_postprocess.sh
- **后处理**: build_b10_sensitivity_table.py, build_kv_ablation_table.py
- **结果目录**: results/emnlp_expansion_v1/
