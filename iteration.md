...
- **状态**：🟢 Tier 0 配置就绪，待远端执行
- **核心假设**：INT4-ours 从未启用 adaptive_static_scales，且 temperature 方向与 INT8 mainline 相反
- **Tier 0**: 2×2 配置网格 smoke test (adaptive × temperature)，仅 32K，3 模型
  - [x] 1.5B 配置: `exp_matrix.yaml` 追加 3 个 `int4_rescue_*` 条目
  - [x] 7B 配置: 新建 `exp_matrix_qwen25_7b_int4_rescue_v1.yaml`
  - [x] 8B 配置: 新建 `exp_matrix_llama31_8b_int4_rescue_v1.yaml` (修正 model_revision)
  - [ ] 远端配置快照备份
  - [ ] 远端校准文件存在性检查 (7B/8B INT4 calib)
  - [ ] 第一波: 三模型 T0-B (adaptive=true, temp=false) PPL+Needle
  - [ ] 第二波: 1.5B 完整 2×2 网格
  - [ ] Go/No-Go 判定
- **Tier 1** (条件): 校准策略重评估 (KL vs MSE vs percentile)
- **Tier 2** (条件): Scale 精度修复 (float32 全链)
